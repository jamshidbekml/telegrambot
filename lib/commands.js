...
const { TikTokVideo, TikTokAudio } = require('../modules/tiktok');
/****************************************** Start Command **************************************/
...
const MessageListener = async (ctx) => {
    try {
        if (ytdl.validateURL(ctx.message.text)) {
            ...
            ProfilePhoto(ctx);
        } else if (ctx.message.text.includes('vt.tiktok.com')) {
            TikTokVideo(ctx)
        } else {
...
const QueryListener = (ctx) => {
    if(ctx.update.callback_query.data.includes('vt.tiktok.com')){
        TikTokAudio(ctx)
    }else {
        YouTubeAudio(ctx);
    }
